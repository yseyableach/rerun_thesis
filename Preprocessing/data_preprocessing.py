# -*- coding: utf-8 -*-
"""
Created on Sun Mar  7 20:16:30 2021
...
the_final=[]   
for corpus in jmir_3inone_sent:
    locate=[]
    kind_of_sentence=[]
...
for i in range(len(final_P_20k_R2CT)):                                         #該層為我要選取第i篇學術文章摘要
    ...
    for j in final_P_20k_R2CT[i]['corpus']:                                    #該層為第i篇學術摘要中的第j個句子
        ...
        for c in range(len(filtered_sentence)):                                #Porter的steeming
            ...
            filtered_sentence[c] = wtlem.lemmatize(filtered_sentence[c])        #wordnet
...
#%%
import pickle
pickle.dump(final_data, open("jmir3inone_.pkl", 'wb'))#這個是save!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
